...
import { daos, DAO } from '@/config/constants'; // Import daos and DAO interface
...
// Define the function
async function fetchOhlcChart(req: Request, res: Response): Promise<void> {
  const { id, days } = req.query as { id?: string; days?: string };

  // Validate `id` and `days` parameters
  if (!id || !days) {
    res.status(400).json({ error: 'Missing required query parameters: id, days' });
    return;
  }
  ...
  try {
    ...
    const cacheKey404 = `${id}-${days}-ohlc-404`;
    ...
    const apiUrl = `https://api.coingecko.com/api/v3/coins/${tokenName}/ohlc?vs_currency=usd&days=${days}`;
